...
const DroneInfo = memo(
  ({
    ...
    return (
      <div className="w-full h-auto p-2 space-y-2">
        {/* 상단: 연결 상태와 배터리 */}
        <div className="text-lg flex items-center justify-between">
          <div className={cls("w-5 aspect-square rounded-full", isOn ? "bg-lime-500" : "bg-neutral-500")} />
          <div className="flex w-full px-2 text-blue-700">
            <div className="font-bold">Co Drone Mini</div>
          </div>
          <p className="flex items-center space-x-1">
            <span className={`font-semibold ${getBatteryColor()}`}>{battery}%</span>
            <span className="flex items-center">
              <FontAwesomeIcon icon={getBatteryIcon()} className={`size-8 ${getBatteryColor()}`} />
            </span>
          </p>
        </div>
        ...
        <div className="text-sm grid grid-cols-3 gap-2 bg-gray-50 p-2 rounded-lg">
          <div className="flex flex-col items-center">
            ...
            <span className="font-semibold">{attitude?.roll?.toFixed(1) ?? "0.0"}°</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-gray-600">Pitch</span>
            <span className="font-semibold">{attitude?.pitch?.toFixed(1) ?? "0.0"}°</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-gray-600">Yaw</span>
            <span className="font-semibold">{attitude?.yaw?.toFixed(1) ?? "0.0"}°</span>
          </div>
        </div>
        ...
        <div className="text-sm grid grid-cols-3 gap-2 bg-gray-50 p-2 rounded-lg">
          <div className="flex flex-col items-center">
            ...
            <span className="font-semibold">{altitude?.toFixed(1) ?? "0.0"} cm</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-gray-600">해발 고도</span>
            <span className="font-semibold">{rangeHeight?.toFixed(1) ?? "0.0"} cm</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-gray-600">온도</span>
            <span className="font-semibold">{temperature?.toFixed(1) ?? "0.0"} °C</span>
          </div>
        </div>
      </div>
    );
